...
const ContentMan = () => {
  ...
  const handleImageUpload = (file) => {
    if (file) {
      ...
      setContentDetails((prevState) => ({
        ...prevState,
        imageSrc: URL.createObjectURL(file), // Create a temporary URL for preview
        body: prevState.body,
      }));
    } else {
      setImageFile(null);
      setContentDetails((prevState) => ({
        ...prevState,
        imageSrc: '', // Reset imageSrc when file is removed
        body: prevState.body,
      }));
    }
  };
  ...
  const handleVideoUpload = (file) => {
    if (file) {
      ...
      setContentDetails((prevState) => ({
        ...prevState,
        videoSrc: URL.createObjectURL(file), // Create a temporary URL for preview
        body: prevState.body,
      }));
    } else {
      setVideoFile(null);
      setContentDetails((prevState) => ({
        ...prevState,
        videoSrc: '', // Reset videoSrc when file is removed
        body: prevState.body,
      }));
    }
  };
