...
router.put('/save-content', upload.fields([
  ...
  try {
    ...
    if (req.files && req.files.video && req.files.video.length > 0) {
      ...
      const chunker = new StreamChunker(videoStream);
      ...
      for await (const chunk of chunker) {
        totalChunks++;
        const videoUploadResult = await cloudinary.uploader.upload_chunked(
          chunk,
          {
            resource_type: 'video',
            // upload_preset: 'your_upload_preset', // Replace with your Cloudinary upload preset
            public_id: `content-videos/${videoFile.originalname}`,
            chunk_number: uploadedChunks,
