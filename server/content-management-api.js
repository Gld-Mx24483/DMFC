...
router.post('/upload-video-chunk', upload.single('chunk'), (req, res) => {
  const { chunkIndex, totalChunks, fileName } = req.body;
  const chunkPath = path.join(UPLOAD_DIR, `${fileName}.${chunkIndex}`);
  ...
  if (Number(chunkIndex) === Number(totalChunks) - 1) {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      fs.unlinkSync(chunkPath); // Remove the individual chunk files
    }

    fs.writeFileSync(videoPath, Buffer.concat(chunks));

    res.status(200).json({ message: 'Video reassembled successfully' });
  } else {
  ...
  try {
    ...
    if (videoFileName) {
      ...
      const videoUploadResult = await cloudinary.uploader.upload(videoFilePath, {
        resource_type: 'video',
        public_id: `content-videos/${videoFileName}`,
      });
      videoPath = videoUploadResult.secure_url;
