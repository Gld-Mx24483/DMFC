...
//   pool.query(updateQuery, [contentId], (error, results) => {
//     if (error) {
//       console.error('Error deleting image file:', error);
//       res.status(500).json({ message: 'Error deleting image file', error: error.message });
//       return;
...
router.put('/save-content', upload.fields([
  ...
  try {
    if (req.files && req.files.image && req.files.image.length > 0) {
      const imageUploadResponse = await cloudinary.uploader.upload(req.files.image[0].path, { resource_type: 'image' });
    ...
    const insertQuery =
      'INSERT INTO content (imageUrl, videoUrl, fullName, title, dateTime, body, uploadTime) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const values = [imageUrl, videoUrl, fullName, title, dateTime, body, uploadTime];
    ...
    pool.query(insertQuery, values, (error, results) => {
      ...
      console.log('Content saved successfully:', results);
      res.status(200).json({ message: 'Content saved successfully!', imageUrl, videoUrl });
    });
  } catch (error) {
    console.error('Error saving content:', error);
    res.status(500).json({ message: 'Error saving content', error: error.message });
  ...
  try {
    ...
    const updateQuery = `UPDATE content 
                         SET imageUrl = COALESCE(?, imageUrl), 
                             ...
                             uploadTime = ?
                         WHERE id = ?`;
    const values = [imageUrl, videoUrl, fullName, title, dateTime, body, uploadTime, id];
    ...
    pool.query(updateQuery, values, (error, results) => {
      ...
      console.log(`Content with ID ${id} updated successfully`);
      res.status(200).json({ message: 'Content updated successfully!', imageUrl, videoUrl });
    });
  } catch (error) {
    console.error('Error updating content:', error);
    res.status(500).json({ message: 'Error updating content', error: error.message });
...
router.get('/get-content', (req, res) => {
  const selectQuery =
    'SELECT id, imageUrl, videoUrl, fullName, title, DATE_FORMAT(dateTime, "%Y-%m-%d") as dateTime, body, uploadTime FROM content';
  pool.query(selectQuery, (error, results) => {
    if (error) {
      console.error('Error fetching content:', error);
      res.status(500).json({ message: 'Error fetching content', error: error.message });
